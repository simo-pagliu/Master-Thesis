...
class ElicitationApp:
    def __init__(self, root):
        ...
        self.asked_025_075 = False  # New flag to track if we've asked about 0.25/0.75 points
    ...
    def set_skip_025_075(self, skip):
        ...
        if not skip:
            self.step += 1
            self.show_step()  # This will show the 0.25/0.75 questions
        else:
            self.step += 1
            self.show_points_before_fit()
    ...
    def save_indifference_05(self):
        try:
            ...
            self.step += 1
            self.plot_points()
            self.ask_skip_025_075()  # Show the skip question after saving 0.5 point
        except ValueError:
            messagebox.showerror("Error", "Please enter valid numbers.")
    ...
    def show_points_before_fit(self):
        ...
        ttk.Label(self.left_frame, text="Points collected:").pack(pady=5)
        for i, (x, y) in enumerate(self.points):
            ttk.Label(self.left_frame, text=f"Point {i+1}: ({x:.2f}, {y:.2f})").pack(pady=2)

        ttk.Button(self.left_frame, text="Next", command=self.next_attribute).pack(pady=5)
        ttk.Button(self.left_frame, text="Back", command=self.prev_step).pack(pady=5)
