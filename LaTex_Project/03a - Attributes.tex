...
Further discussion with several experts from different background was helpful to identigy other possible aspects as well as to collect feedback that helped improving the justifications. On this regard it is really important to not theat the analyst shall always take into serious consideration every suggestion provided by others, and, whenever these have to be discarted, it is important to report it and properly justify the reasons to ensure full transparency.
...
\begin{itemize}
    \item Double counting: The same aspcets might be represented by multiple indicators, leading to biased weights
    ...
    \item Data availability: This wouldn't be the case for a professional application but for this thesis obtaining data outside of the public domain was deemed out of scope.
\end{itemize}

Table \ref{tab:screening-indicators} provides examples of excluded indicators. The most significant exclusions are justified in Section \ref{sec:indicators_removed} while possible improvements for future studies are documented in Section \textcolor{red}{PLACEHOLDER}.
