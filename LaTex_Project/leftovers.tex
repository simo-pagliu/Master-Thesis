...
In this section, limitations of the present study and consequent suggestions for future improvements and extensions to this work are provided. These suggestions are divided into three categories: improvements to the case study, improvements to the elicitation process, and improvements to the \gls{upmavt} method. Further suggestions that emerged during the development of this thesis but were considered out of scope are collected in Appendix~\ref{sec:appendix_suggestions}.
...
Biodiversity impacts are intentionally excluded, on the assumption that preliminary site screening would eliminate protected areas and high-value ecosystems from consideration. All environmental indicators should reflect regulatory agency perspectives rather than public perception metrics.

At the national level, incorporating an import-reduction objective could yield valuable strategic insights. This would enable direct comparison of deployment configurations, such as evaluating a single large reactor against several \glspl{smr} delivering equivalent capacity, while maintaining focus on energy security outcomes.
...
Such a system would simultaneously reduce participant burden and enhance robustness of results through broader engagement of specialists.

\paragraph{Effectiveness of correction approach to value function elicitation}
...
\paragraph{Criticalities among qualitative indicators}
...
\paragraph{Effectiveness of the PILE-BWT graphical support}
The digital interface significantly facilitated the elicitation process, though expert guidance by the analyst remained essential for proper method application. The primary challenge was the inherent difficulty of quantifying pairwise comparisons, a task demanding substantial cognitive effort that should be accounted for in session planning.

\paragraph{Time-saver range adjustment on PILE-BWT results requires further analysis for validation}
A methodological compromise was necessary for the \gls{pilebwt} process. Ideally, comparisons should have been country-specific to account for varying indicator ranges (country-dependent indicators). To balance rigour with practical constraints, all comparisons used unified ranges spanning minimum-maximum values across all countries. Post-elicitation, country-specific values were derived through linear mapping to preserve relative relationships. This approach maintains value function validity within each national context, unlike range clipping that could distort relationships. While no significant bias is expected from this simplification, dedicated validation studies would be valuable to confirm this assumption.
