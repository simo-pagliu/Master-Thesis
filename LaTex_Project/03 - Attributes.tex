...
Given the bottom up approach we presented in \textcolor{red}{Section PLACEHOLDER}, the first step of this analysis is to try and identify all possible ways to differentiate between the six alternatives under study. The process started with brainstorming from the author who then applied a first selection. This was then presented to X experts asking for missing, repetitive, useless, too aggregate or too specific indicators, this lead to a much longer list of posisble indicators which is presented in \textcolor{red}{Table PLACEHOLDER}.


\subsection{Screening the Indicators}
As previously anticipated the large list of indicators has to be screened, not only it would be too time consuming to obtain all that data but it would also be pointless, as we are going to discuss in the following paragraphs many inidcators might be redunant as they are highly correlated. ANother factor to consider is the relevance to the present study, we are focusing on a govermental prospective in europe, therefore something like the return on investment by operating the reactor would not be an interesting parameter given that the reactor will be operated by private equity. At last some indicators where discarted due to the unavability of publicly availble data or whenever the effort needed to obtain information was deemed out of the scope of this thesis. Everything is therefore documented hereafter as possible improvements for future studies, on this reagard please also read \textcolor{red}{Section conclusions - future improvments PLACEHOLDER}.
...
In the end we are left with X indicators, which had to be aggregated into higher level objectives...
