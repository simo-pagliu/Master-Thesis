...
Best criteria comparison: we now do pairwise comparison using $C_b$ as our reference, in our four criteria example let's say that $C_1$ is $C_b$. We will ask experts to tell us the value $x$ that $C_b$ should have so that a pair of hypotetical consequences is indifferent: Determine $x$ such that $(x,0,0,0)$ is indifferent to $(0,1,0,0)$, the result will be noted as $x_{(b,2)}$, then the same will be done for $(0,0,1,0)$ to determine $x_{(b,3)}$ and finally $x_{(b,4)}$ from $(0,0,0,1)$.
...
\begin{equation}
   ...
   w_1 = \frac{w_2}{v_2(x_{(b,2)})} = \frac{w_3}{v_3(x_{(b,3)})} = \frac{w_4}{v_4(x_{(b,4)})}
\end{equation}

Then the procedure is repeated for $C_w$, which we assume is $C_4$, so that we ask to determine $x$ so that:
$x$ so that $(0,0,0,1)$ is equivalent to 
\begin{itemize}
\item $(x,0,0,0)$ will determine $x_{(w,1)}$
\item $(0,x,0,0)$ will determine $x_{(w,2)}$
\item $(0,0,x,0)$ will determine $x_{(w,3)}$
\end{itemize}
...
By breaking up the relationship in \ref{eq:worst_comparison} and \ref{eq:best_comparison} and by adding the normalization condition $\sum_{i=1}^{n} w_i = 1$ we can obtain a linear system of equations.

\begin{equation}
   \begin{cases}
      w_i \cdot v_i(x_{(w,i)}) = w_w \forall i \in \{1,2,3\} \\
      w_b = \frac{w_i}{v_i(x_{(b,i)})} \forall i \in \{2,3,4\} \\
      \sum_{i=1}^{4} w_i = 1
   \end{cases}
\end{equation}
...
\begin{equation}
   minimize \max \left\{
   \begin{array}{l}
      |w_i \cdot v_i(x_{(w,i)}) - w_w| \quad \forall i \in \{1,2,3\} \\
      |w_b - \frac{w_i}{v_i(x_{(b,i)})}| \quad \forall i \in \{2,3,4\}
   \end{array}
   \right\}
   subject \; to \quad \sum_{i=1}^{4} w_i = 1
   and \quad w_i \geq 0 \quad \forall i
\end{equation}
